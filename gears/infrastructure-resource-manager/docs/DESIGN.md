...
The redispatch tick keeps a stuck operation moving; a second, separate background check is what keeps every operation reaching a terminal state at all (`cpt-cf-infrastructure-resource-manager-fr-lifecycle-states`). On the same deployment-configurable tick, and under the same fenced lease from persisted state alone, this component claims every non-terminal operation whose age exceeds the running-operation maximum lifetime published in `cpt-cf-infrastructure-resource-manager-nfr-limits` and transitions it to terminal `failed` with a distinct `max_lifetime_exceeded` reason. The check is deliberately blind to kind and to current state: it covers a still-`pending`, never-dispatched operation exactly as it covers a `running` one, and it covers refresh and discovery operations, which have no forward-apply call that could reject them. That terminal transition is a failure like any other for quota purposes. It records any surviving allocation as usage and returns only unused capacity. Later cleanup reverses or credits the recorded usage as each surviving allocation is removed (`cpt-cf-infrastructure-resource-manager-fr-quota-gating`). The synchronous dispatch-deadline rejection on the apply path stays the fast path for an operation a caller returns to; this backstop is what makes the terminal-state guarantee unconditional for the operations no caller ever returns to.
...
- **121/121 PRD cpt IDs traced**: 6 human actors, 12 system actors, 77 functional requirements, 11 non-functional requirements, 3 public interfaces, 3 external contracts, 9 use cases. The tables use the common-prefix shorthand defined at the start of §5.
