...
A dry run follows the same path and suppresses entity-state commit. Its mode is part of the fingerprint and is copied to candidate rows so `ck_tr_operation_item_state` can constrain its result fields. The worker predicts the whole batch against one read snapshot: each candidate runs the ordinary commit-time reads, write-order claim and rechecks through an in-memory overlay that the next candidate reads, so the reads still reach the database while no entity-state write statement is issued and the write-order claim is virtual, taken against no other writer. It records the predicted outcomes in a separate short transaction once that snapshot is released. The verdict is relative to that observed state, not a reservation; §3.3 defines the one result-shape difference.
