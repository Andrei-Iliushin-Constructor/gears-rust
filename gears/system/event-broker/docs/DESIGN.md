...
The event broker SDK defines the storage backend contract following the `Backend` contract type semantics from [PR #1536](https://github.com/constructorfabric/gears-rust/issues/3622) — remote-capable, independent failure domain, errors as RFC 9457 Problem Details, `SecurityContext` as first argument on every method.
