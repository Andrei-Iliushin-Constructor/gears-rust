...
sequenceDiagram
    ...
    alt Root tenant exists in active status
        ...
        TS->>TS: Resolve root_type.gts_id via GTS effective traits and require root eligibility
        rect rgb(230, 245, 255)
            Note over TS,DB: Saga step 1 — short TX
            TS->>DB: INSERT tenant (root, type=root_type.gts_id, status='provisioning', self_managed)
        ...
        TS->>IDP: provision_tenant(IdpProvisionTenantRequest{ tenant_id=root_id, tenant_name, tenant_type=root_type.gts_id, tenant_metadata=root_tenant_metadata, parent_id=None })
...
The AM-owned contract is configured independently from the optional bootstrap saga and is reconciled during `init`. Schema drift, ownership conflicts, and an existing root whose durable type UUID differs are always startup-fatal; `bootstrap.strict` applies only to the later tenant-creation saga. During migration, `bootstrap.root_tenant_type` and `bootstrap.root_tenant_type_idp_provisioning` remain deprecated aliases and must agree with `root_type` when both forms are present.
...
|-----------|------|----------|-------------|
| `root_type.gts_id` | string (chained `GtsSchemaId`) | Required when bootstrap is enabled | Canonical concrete schema identifier with exactly one concrete segment directly derived from `gts.cf.core.am.tenant_type.v1~`. AM creates it in authoritative Types Registry storage if absent and semantically validates it if present. Deployment-specific — e.g., `gts.cf.core.am.tenant_type.v1~cf.core.am.provider.v1~`. |
| `root_type.idp_provisioning` | boolean | No (default: `false`) | Desired effective `idp_provisioning` trait for the create-once root-type contract. |
