...
|-----------|------|----------|-------------|
| `root_type.gts_id` | string (chained `GtsSchemaId`) | Required when bootstrap is enabled | Canonical concrete schema identifier derived from `gts.cf.core.am.tenant_type.v1~`. AM creates it in authoritative Types Registry storage if absent and semantically validates it if present. Deployment-specific — e.g., `gts.cf.core.am.tenant_type.v1~cf.core.am.provider.v1~`. |
