...
A reference whose target no longer exists **MUST** keep resolving as a valid value: it **MUST NOT** be flagged `needs-review`, **MUST NOT** be excluded from Apply, and the service **MUST NOT** be required to detect the deletion at all. A dangling reference is discovered by the consumer when it fetches, from the file storage.
