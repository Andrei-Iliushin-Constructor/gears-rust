...
> **Note**: Validation of the reference was considered and deliberately dropped (design review, 2026-08-24), which reverses three properties requested in review — declared content-type/size constraints checked at staging, refusing an Apply whose reference no longer resolves, and a `needs-review` state for a dangling applied reference. Version pinning and classification handling are kept. The trade is that this service gains no dependency on the file storage at all: no client to add, no capability to wait on, no cross-gear prerequisite. Recorded here so the decision is visible rather than re-litigated.
...
Setting a value **MUST** require credential re-verification (step-up).

- **Rationale**: An administrator must be able to see what a change would do before making it, and must not be able to change a live platform by surprise.
- **Actors**: `cpt-cf-settings-service-actor-platform-admin`, `cpt-cf-settings-service-actor-tenant-admin`
