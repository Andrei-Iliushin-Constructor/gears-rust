...
- Setting **Declaration** lifecycle — admin-authored and **module-contributed** (register/retire). The setting's **key** is a **GTS instance identifier** of the form `<value-type>~<setting-instance-id>` for **both** authors: the left segment is the curated **value type** the setting derives its shape from (`gts.cf.toolkit.settings.type_*~`, registered in the Registry), and the right segment is the setting's own instance id (no trailing `~`). Only **value types** are registered in GTS; the setting itself is a GTS **instance** and is **not** registered — it lives in the Settings DB. There is no separate `gts_type_id` (the value type is the left half of the key). The declaration is kept separate from the value.
...
The setting `key` is a **GTS instance identifier** `<value-type>~<setting-instance-id>` for **both** authors. Left = the curated **value type** (`gts.cf.toolkit.settings.type_*~`, the only thing registered in GTS). Right = the setting's instance id (no trailing `~`): a **module** provides its full GTS id and the category is **extracted from the `<namespace>` slot**; an **admin** setting's instance id is `gts.<vendor>.settings.<category>.<name>.v1`, where `<category>` occupies that same `<namespace>` slot and `<vendor>`/`<name>` are the admin's inputs. Both forms are **four segments** before the version — `<vendor>.<package>.<namespace>.<type>` per the GTS grammar (§4.7) — with the category always in the third. The setting is a GTS **instance** and is **not** registered; there is no separate `gts_type_id` field (the value type is the left half of the key).
...
These control-plane types are **separate** from **setting keys** and **value types**. A setting `key` is a GTS **instance** id `<value-type>~<instance-id>` (§4.1); its **instance** half is authored by the *deploying party* (module or admin) and its vendor is that party's — not necessarily `cf` — see §4.2 *Declaration Management* / *Contribution Reconciler* and §4.7. Its **value-type** half is a curated catalog type under `gts.cf.toolkit.settings.type_*~` (the only GTS-registered part).

**Policies may target settings by GTS type id, wildcards included.** Because a setting's `key` carries its value type in the left half, a policy can be written against that type rather than against each key — `gts.cf.toolkit.settings.type_connection_string.v1~*`, for instance, to govern every setting of that type at once. Matching is the **platform authorization layer's** to perform: this gear passes the key as the resource and does not interpret patterns itself, so wildcard semantics stay owned by `authz-resolver` and cannot drift into a second, gear-local matcher (`cpt-cf-settings-service-fr-authn-role-gating`).
...
| `key` | string | Yes | Globally-unique setting id passed to `resolve` and used in REST value paths — a **GTS instance identifier** `<value-type>~<setting-instance-id>` (chain form) for **both** authors (§4.2 *Declaration Management*, *Contribution Reconciler*). Left segment = the curated **value type** (`gts.cf.toolkit.settings.type_*~`, ends with `~`, the only GTS-registered part). Right segment (no trailing `~`) = the setting instance id: for a **module** setting, the id the gear supplies, whose namespace segment names the category (§4.2 *Contribution Reconciler*); for an **admin** setting, `gts.<vendor>.settings.<category>.<name>.v1` (§4.2 *Declaration Management*). The setting is a GTS **instance**, not a registered type. Consumers treat the whole `key` as an opaque identifier. |
...
| `value_type_id` | string | Yes | GTS id of the **value type** the value is validated against — a curated type from the catalog `gts.cf.toolkit.settings.type_*~`, **registered** in GTS. This is exactly the **left half** of `key`. Carries the trait set (incl. `secret`). |
...
- `{key}` is the `SettingDeclaration.key` (§4.1) — a GTS instance identifier `<value-type>~<instance-id>` (both authors). It contains `~` (and, in the instance segment, `.`), which does not collide with the `:`/`@` audit-resource delimiters, so the id stays parseable. Because the key is immutable (§4.1), a setting's per-(setting, scope) history is **continuous** for the life of the declaration: no metadata edit changes the `resource` id, so no trail is ever split by an administrative change.
...
Two things in that shape are load-bearing rather than stylistic. The typestate forces an auth posture, and **every route in this gear declares `.authenticated()`** — there is no `.anonymous()` route, which is the same statement §4.8 makes about the authorization model, expressed where it is mechanically enforced. And `operation_id` is the gear-prefixed `settings_service.<verb>`, matching `types_registry.register` and its siblings, so generated clients namespace cleanly.
...
> **Why not `resource:verb`.** An earlier draft of this document used a colon (`applies:preview`, `pending/{key}:revert`). Nothing in the platform rejects one — axum 0.8 spells parameters `{name}`, so a colon is an ordinary literal character inside a segment, and `axum_to_openapi_path` rewrites only `{*`, so the colon would reach the generated OpenAPI intact. But **no `OperationBuilder` declaration in the workspace uses one**, and no platform guideline prescribes the convention: it was this document's own invention. Plain segments cost nothing and match every other gear.
...
| `GET` | `/settings-service/v1/declarations/{id}` | Get a declaration (incl. `value_type_id` = key's left half + resolved `traits`) | Yes |
...
| `GET` | `/settings-service/v1/me/preferences` | Read the caller's per-user mode preference | Yes |
| `PUT` | `/settings-service/v1/me/preferences` | Persist the caller's per-user mode preference (per user, not per session; `cpt-cf-settings-service-fr-standard-advanced-mode`) | Yes |
...
| `value_type_id` | text | No | — | GTS id of the value type (catalog `gts.cf.toolkit.settings.type_*~`); this is the **left half** of `key` (both authors, §1.3) |
...
>
> - **Module** settings: the gear supplies the instance id; its vendor/package segments follow **who authored the gear** — CF-authored modules use `gts.cf.settings.<category>.<name>.v1`, third-party-vendor modules use `gts.<vendor>.<vendor_package>.<category>.<name>.v1`. The `<category>` segment is the category the setting belongs to (extracted by the reconciler — §4.2 *Contribution Reconciler*).
> - **Admin** settings: the instance id is `gts.<vendor>.settings.<category>.<name>.v1` — `<vendor>`/`<name>` entered by the admin, `<category>` = the category it was created in (§4.2 *Declaration Management*).
>
...
| Validate a value (`POST /settings-service/v1/settings/{key}/validate`) | `read` on `gts.cf.toolkit.settings.value.v1~` — **no step-up** (read-only, §4.2 *Value Writer*) | Own scope subtree | `403` |
...
| Read/set own mode preference (`GET`/`PUT /settings-service/v1/me/preferences`) | Authenticated caller, **own record only** — `user_id` forced to the token subject; no cross-user access (§4.3) | Self | `401` (no valid bearer) |
...
- **Setting discovery — RESOLVED: settings are listed from the Settings DB, value types from the Registry.** Since a setting is a GTS **instance** in our own tables (not a registered type), "list all settings" is a Settings-DB query (§4.3 `list_declarations`), not a Registry listing. What the Registry lists is the small curated **value-type** catalog (`gts.cf.toolkit.settings.type_*~`) — the shapes an admin may pick from. Eligibility is built in: `create_declaration`/`register_declarations` reference a catalog value type via `value_type_id` (the key's left half), so an arbitrary/inappropriate shape cannot slip in; a genuinely new shape is a reviewed design-time addition to the catalog.
- **Setting identity — RESOLVED: key is a GTS *instance* id `<value-type>~<instance-id>` for both authors**. Only **value types** (`gts.cf.toolkit.settings.type_*~`) are registered in GTS; a setting is a GTS **instance** and is **not** registered (per the [GTS spec](https://github.com/globaltypesystem/gts-spec): instance registration is not mandated). Both module and admin settings are uniformly GTS-referenceable by their instance id.
  - **Key = `<value-type>~<instance-id>`.** Left half = the curated value type the value conforms to (the only registered part). Right half = the setting's instance id: a **module** supplies its own id (`gts.<vendor>.[<pkg>|settings].<category>.<name>.v1`), and the reconciler extracts `<category>` from the namespace segment (§4.2 *Contribution Reconciler*); an **admin** id is `gts.<vendor>.settings.<category>.<name>.v1`, `<vendor>`/`<name>` entered by the admin, `<category>` = the target category (§4.2 *Declaration Management*).
- **Dual addressing (declarations by UUID, values/history by key) — the decision stands, its written rationale does not.** The original argument was that the `key` is a **mutable attribute** re-keyed by a category rename or move, so the management plane could not address a resource by the attribute it mutates. That is not the case: no operation re-keys a declaration (§4.1), so the argument is void and has been removed rather than patched. The split itself is unchanged and visible in §4.3 — consumers read by `key`, the console edits by UUID — but **why** it is the right split needs stating afresh, which is not this review thread's subject.
  ...
  - **The category binding is fixed, and that is a closed door rather than a trade — RESOLVED (review, MikeFalcon77).** The category is part of the setting's identity (`network.timeout` vs `database.timeout`), so it sits in the key; presentation lives on the category row (`sort_order`, `icon`, display `name`). Consequently **a category slug cannot be renamed and a setting cannot be moved between categories**: `update_category` does not expose the slug, `update_declaration` does not re-bind `category_id`, and `key` is rejected in a `PATCH`. Reviewed as a risk to consumer reads; it is not one, because no operation performs that transition. Should product later want re-shelving, the path is a new declaration under a new key plus a value migration — a scoped addition, not a gap.
...
| Mode preferences round-trip | `GET`/`PUT /settings-service/v1/me/preferences` | Persisted per user; mode filter applied to browse/search (`cpt-cf-settings-service-fr-standard-advanced-mode`) |
