...
| Read activation status (`GET /v1/change-sets/{change_set_id}/activation`) | `read` on `gts.cf.toolkit.settings.change_set.v1~` | The change set's tenant subtree | `404` when not visible (activation §4.8) |
