...
| Stage override (set/revert/remove) | `write` on the **setting's key** (§4.7); **for a tenant caller** also `tenant_overridable` (see the note below) | Target scope, within the caller's subtree | `403`/`409` |
