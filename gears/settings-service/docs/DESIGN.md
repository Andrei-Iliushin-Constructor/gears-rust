...
Contents: the out-of-process deployment profiles; subject-scope resolution on top of R1's identity model; file-valued settings (`cpt-cf-settings-service-fr-file-valued-settings`); cross-field search (`cpt-cf-settings-service-fr-search-discoverability`); mode-filtered reads (`cpt-cf-settings-service-fr-standard-advanced-mode`); feature and licence gating (`cpt-cf-settings-service-fr-feature-license-gating`); tenant-deleted cleanup; and **step-up on the platform-wide elevated session**, bound behind the existing `StepUpVerifier` port (§4.2 *Value Writer*). R1's own binding carries step-up until then, so this is a substitution rather than a gap being filled — but it is a **weakening** one, and adopting it requires the PRD to relax `cpt-cf-settings-service-fr-validate-before-set` from re-verification before *every* write to re-verification before every write *outside an open elevated window*. That amendment is a prerequisite of this item, not a consequence of it.
...
**It is a weaker guarantee, not merely a different mechanism, and that has a prerequisite.** An elevated session covers every set inside its window with one ceremony, while `cpt-cf-settings-service-fr-validate-before-set` requires re-verification before **every** set and carries no carve-out. The port makes the swap cheap, but it does not make the requirement follow: **the PRD must first relax that clause** to re-verification before every set *outside an open elevated window*. Until it does, this design binds the R1 verifier, which is why R1 does not depend on the session existing.
