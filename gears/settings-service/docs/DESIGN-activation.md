...
If the referenced file is deleted while an apply is in flight, the subscriber cannot self-react and its await-record follows the ordinary un-acknowledged path — it stays `awaiting` and the `apply_notification` is re-delivered on the usual interval, because the tracker never moves a record to a terminal state on a timer (§4.2 *Apply Outcome Tracker*). This is the same behaviour as for any consumer that cannot apply. Nothing else happens: the reference is stored unvalidated and no deletion signal is consumed (DESIGN.md §3 *Files*), so the value stays valid, stays applicable, and the only party that learns the file is gone is the subscriber that tried to fetch it.
...
| `deliver_awaiting` | — | — | The **delivery loop**: publishes a filtered `apply_notification` per subscriber from its `awaiting` records — immediately after the co-commit (prompt) and on a periodic reconcile scan (retry, broker-outage recovery, and post-restart re-delivery). Honours `redeliver_interval_seconds` via `last_notified_at`. Stops for a record once it is terminal (§4.2 *Apply Outcome Tracker*). **Every replica runs this loop, and the periodic scan is serialised across them by an advisory lock** — see *Why only one replica scans at a time*. |
